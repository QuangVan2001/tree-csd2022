...
public class BST_Fish_Tree {
    ...
    public BST_Node add_recur(BST_Node curref, AquariumFish fish) {
        if (curref == null) {
            ...
            curref.right = add_recur(curref.left, fish);
        }
        return curref;
    }
    ...
    public BST_Node search_Recurr(BST_Node curref, AquariumFish fish) {
        if (curref == null) {
            return null;
        } else {
            ...
            if (d == 0) {
                ...
                return search_Recurr(curref.left, fish);
            } else {
                return search_Recurr(curref.right, fish);
            }
        }
    }

    public BST_Node search_Recur(AquariumFish fish) {
        return search_Recurr(root, fish);
    }
    ...
    public void BF_Traverse_F(String filename) throws FileNotFoundException {
        if (this.isEmpty()) {
            System.out.println("Empty Trê");
            return;
        }
    ...
    public void NLR_F(PrintWriter pw, BST_Node p) {
        if (p != null) {
            try {
                visitF(pw, p);
                NLR_F(pw, p.left);
    ...
    public void NLR_F(String filename) throws FileNotFoundException {
        if (this.isEmpty()) {
            System.out.println("Empty Tree");
        } else {
            PrintWriter pw = new PrintWriter(filename);
    ...
    public void LNR_F(PrintWriter pw, BST_Node p) {
        if (p != null) {
            try {
                LNR_F(pw, p.left);
                visitF(pw, p);
    ...
    public void LNR_F(String filename) throws FileNotFoundException {
        if (this.isEmpty()) {
            System.out.println("Empty Tree");
        } else {
            PrintWriter pw = new PrintWriter(filename);
    ...
    public void RNL_F(String filename) throws FileNotFoundException {
        if (this.isEmpty()) {
            System.out.println("Empty Tree");
        } else {
            PrintWriter pw = new PrintWriter(filename);
    ...
    private BST_Node deleteLeaf(BST_Node father, BST_Node deleteChild){
        if(deleteChild== root)  root= null;
        else{
            if(father.left == deleteChild) father.left= null;
    ...
    private BST_Node deleteOneChildNode(BST_Node father, BST_Node deleteChild){
        if(deleteChild==root){
            ...
            BST_Node grandChild;
            if(deleteChild.left != null) grandChild= deleteChild.left;
            else grandChild =deleteChild.right;
            if(father.left == deleteChild) father.left = grandChild;
